...
def plot_results(returns_matrix_list, subtitle_name, save_path,algo, window=100):
    """
    ...
    for i, returns_matrix in enumerate(returns_matrix_list):
        ...
        if algo == 'Duel_DQN':
            ax1.set_title(f'Type {i+1} - Episodic Returns')
            
        elif algo == 'MC_RF' and i == 0:
            ax1.set_title(f'Without Baseline - Episodic Returns')
            
        elif algo == 'MC_RF' and i == 1:
            ax1.set_title(f'With Baseline - Episodic Returns')
        ...
        if algo == "Duel_DQN":
            ax2.set_title(f'Type {i+1} - Moving Average Episodic Returns')
            
        elif algo == 'MC_RF' and i == 0:
            ax2.set_title(f'Without Baseline - Moving Average Episodic Returns')
            
        elif algo == 'MC_RF' and i == 1:
            ax2.set_title(f'With Baseline - Moving Average Episodic Returns')
...
# Plotting the episodic_return and mean_episodic_return for Dueling DQN type 1 & 2 on Acrobot environment 

# Loading all lists from the pickle file
with open('best_config/Duel_DQN_t1_best_config.pkl', 'rb') as file:
    all_returns1, all_regrets1 = pickle.load(file)

with open('best_config/Duel_DQN_t2_best_config.pkl', 'rb') as file:
    all_returns2, all_regrets2 = pickle.load(file)
    

# Episodic return data for 3 configurations (each is a 5 x 10000 matrix)
returns_matrix_list = [np.array(all_returns1), np.array(all_returns2)]

# Plotting the graphs
plot_results(returns_matrix_list,subtitle_name= 'Acrobot_Dueling_DQN_Plots',save_path = 'best_param_plots/Acrobot_Dueling_DQN_Plots.png',algo = 'Duel_DQN',window=100)

    

# Plotting the episodic_return and mean_episodic_return for MC Reinforce with and without baseline on Acrobot environment 

# Loading all lists from the pickle file
with open('best_config/MC_rf_without_bl_best_config.pkl', 'rb') as file:
    all_returns1, all_regrets1 = pickle.load(file)

with open('best_config/MC_rf_w_bl_best_config.pkl', 'rb') as file:
    all_returns2, all_regrets2 = pickle.load(file)
...
# Plotting the graphs
plot_results(returns_matrix_list,subtitle_name= 'Acrobot_MC_Reinforce_Plots',save_path = 'best_param_plots/Acrobot_MC_Reinforce_Plots.png',algo='MC_RF',window=100)